...
export function parsePositiveInt(value: string, optionName: string): number {
  ...
  if (n < 1) {
    throw new Error(`Invalid ${optionName}: "${value}". Must be at least 1`);
  }
  return n;
}
...
export type SortField = 'name' | 'game-pct' | 'global-pct' | 'progress' | 'points';
...
export function validateStatus(v: string): AchievementStatus {
  if (v !== 'completed' && v !== 'uncompleted') {
    throw new Error(`Invalid status: "${v}". Must be "completed" or "uncompleted"`);
  }
  return v;
}

export function validateSort(v: string): SortField {
  const valid: SortField[] = ['name', 'game-pct', 'global-pct', 'progress', 'points'];
  if (!valid.includes(v as SortField)) {
    throw new Error(`Invalid sort: "${v}". Must be one of: ${valid.join(', ')}`);
  }
...
export function resolveGameFilter(gameArg: string, games: string[]): string {
  ...
  if (!match) {
    throw new Error(`Unknown game "${gameArg}". Available: ${games.join(', ')}`);
  }
  return match;
}
