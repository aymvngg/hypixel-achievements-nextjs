...
function toCamelCase(snake: string): string {
  return snake.toLowerCase().replace(/_([a-z])/g, (_, c: string) => c.toUpperCase());
}
...
function tierObtainedPoints(tierInformation: unknown, currentTier: number): number {
  const ti = tierInformation as { tierInfo?: Array<{ points?: number }> } | null | undefined;
...
export function correlateAchievements(
  ...
  for (const [, gameData] of Object.entries(achievements.achievementsPerGame)) {
    ...
    for (const achievement of ga.achievements) {
      ...
      const prefixedKey = toCamelCase(`${ga.category}_${achievement.codeName}`);
      const playerVal =
        player.tieredAchievements[prefixedKey] ?? player.tieredAchievements[bareKey] ?? 0;
      ...
      const completed = isTiered ? currentTier >= maxTier : oneTimeSet.has(oneTimeKey);
      const progress = isTiered && maxTier > 0 ? currentTier / maxTier : completed ? 1 : 0;
