...
function Section({
	...
	return (
		<section className="space-y-2">
			<h2
				className={`${PF} text-mc-sky uppercase tracking-wider text-sm`}
			>
				{title}
			</h2>
			{children}
		...
		<div className={`${PANEL} px-4 py-3`}>
			<div className="flex items-center justify-between gap-3">
				<div className="flex items-center gap-3 min-w-0">
					...
					<div>
						<p
							className={`${PF} truncate ${p1Wins ? "text-mc-gold" : "text-mc-stone-light"}`}
						>
							{P1.name}
						</p>
						<p
							className={`${PF} text-lg text-mc-grass tabular-nums`}
						>
							{P1.total.toLocaleString()}
						</p>
					</div>
				</div>
				...
				<div className="flex items-center gap-3 min-w-0 flex-row-reverse text-right">
					...
					<div>
						<p className={`${PF} truncate text-mc-stone-light`}>
							{P2.name}
						</p>
						<p
							className={`${PF} text-lg text-mc-grass tabular-nums`}
						>
							{P2.total.toLocaleString()}
						</p>
					</div>
				</div>
			</div>
...
function SummaryB() {
	return (
		<div
			className={`${PANEL} grid grid-cols-2 divide-x-2 divide-mc-border`}
		>
			{[
				...
				<div key={i} className="p-3 flex items-center gap-3">
					...
					<div>
						<p
							className={`${PF} ${s.wins ? "text-mc-gold" : "text-mc-stone-light"}`}
						>
							{s.p.name}
						</p>
						<p
							className={`${PF} text-xl text-mc-grass tabular-nums`}
						>
							{s.p.total.toLocaleString()} AP
						</p>
						<p className="text-[0.6rem] text-mc-stone-light">
							Completion 52%
						</p>
					</div>
				</div>
...
function SummaryC() {
	...
	return (
		<div
			className={`${PANEL} px-4 py-3 flex items-center justify-center gap-4`}
		>
			<span className={`${PF} text-3xl text-mc-grass tabular-nums`}>
				{P1.total.toLocaleString()}
			</span>
			<div className="text-center">
				<p className={`${PF} text-xs uppercase text-mc-stone-light`}>
					diff
				</p>
				<p className={`${PF} text-xl text-mc-gold tabular-nums`}>
					+{diff.toLocaleString()}
				</p>
				<p className={`text-[0.6rem] ${PF} uppercase text-mc-grass`}>
					{lead} leads
				</p>
			</div>
			<span className={`${PF} text-3xl text-mc-grass tabular-nums`}>
				...
				<thead>
					<tr className="bg-mc-panel border-b-[3px] border-mc-border">
						...
						<th className="text-left px-2 py-1.5">
							<span
								className={`${PF} text-xs uppercase text-mc-stone-light tracking-wider`}
							>
								Game
							</span>
						</th>
						<th className="text-right px-2 py-1.5">
							<span
								className={`${PF} text-xs uppercase text-mc-sky tracking-wider`}
							>
								{P1.name}
							</span>
						</th>
					...
					{ROWS.map((r, i) => {
						...
						return (
							...
							>
								...
								<td
									className={`px-2 py-1.5 ${PF} text-xs text-mc-gold truncate`}
								>
									{formatGameLabel(r.game)}
								</td>
								<td
									className={`px-2 py-1.5 text-right tabular-nums ${diff > 0 ? "text-mc-grass" : "text-mc-stone-light"}`}
								>
									{r.p1Obtained.toLocaleString()}
								</td>
								<td
									className={`px-2 py-1.5 text-right tabular-nums ${diff < 0 ? "text-mc-grass" : "text-mc-stone-light"}`}
								>
									{r.p2Obtained.toLocaleString()}
								</td>
								<td
									className={`px-2 py-1.5 text-right tabular-nums text-xs ${diff > 0 ? "text-mc-grass" : diff < 0 ? "text-mc-red" : "text-mc-stone-light"}`}
								>
									{diff > 0 ? "+" : ""}
									{diff.toLocaleString()}
								</td>
				...
				return (
					<div key={r.game} className={`${PANEL} overflow-hidden`}>
						<div className="flex items-center gap-2 bg-mc-panel border-b-[3px] border-mc-border px-2.5 py-1.5">
							...
							<h3
								className={`${PF} text-sm text-mc-gold uppercase tracking-[0.04em] truncate`}
							>
								{formatGameLabel(r.game)}
							</h3>
						</div>
						<div className="flex divide-x-2 divide-mc-border">
							<div className="flex-1 min-w-0 p-2.5">
								<p
									className={`${PF} text-xs uppercase ${p1Wins ? "text-mc-grass" : "text-mc-stone-light"}`}
								>
									{P1.name}
								</p>
								<p
									className={`${PF} text-mc-gold text-base tabular-nums`}
								>
									{r.p1Obtained.toLocaleString()} AP
								</p>
								<p className="text-[0.65rem] text-mc-stone-light">
									{r.p1Completed}/{r.p1Count} done
								</p>
							</div>
							<div className="flex-1 min-w-0 p-2.5 text-right">
								...
								<p
									className={`${PF} text-mc-gold text-base tabular-nums`}
								>
									{r.p2Obtained.toLocaleString()} AP
								</p>
								<p className="text-[0.65rem] text-mc-stone-light">
									{r.p2Completed}/{r.p2Count} done
								</p>
							</div>
						</div>
					</div>
			...
			{ROWS.map((r) => {
				...
				return (
					<div key={r.game} className={`${PANEL} flex`}>
						...
						<div className="flex-1 min-w-0 p-2.5">
							<h3
								className={`${PF} text-sm text-mc-gold uppercase truncate`}
							>
								{formatGameLabel(r.game)}
							</h3>
							<div className="flex justify-between items-baseline mt-1">
								<span
									className={`${PF} text-xs ${diff >= 0 ? "text-mc-grass" : "text-mc-stone-light"}`}
								>
									{P1.name} {r.p1Obtained.toLocaleString()}
								</span>
								<span
									className={`${PF} text-xs ${diff <= 0 ? "text-mc-grass" : "text-mc-stone-light"}`}
								>
									{r.p2Obtained.toLocaleString()} {P2.name}
								</span>
							</div>
							<p
								className={`text-[0.6rem] ${PF} ${diff > 0 ? "text-mc-grass" : diff < 0 ? "text-mc-red" : "text-mc-stone-light"}`}
							>
								{diff > 0
									? `${P1.name} +${diff.toLocaleString()}`
									: diff < 0
										? `${P2.name} +${Math.abs(diff).toLocaleString()}`
										: "tied"}
							</p>
						</div>
					</div>
				...
				return (
					<div key={r.game} className={`${PANEL} p-2.5`}>
						<div className="flex items-center gap-2 mb-2">
							...
							<h3
								className={`${PF} text-sm text-mc-gold uppercase truncate flex-1`}
							>
								{formatGameLabel(r.game)}
							</h3>
						</div>
						<div className="flex justify-between items-baseline mb-1">
							<span
								className={`${PF} text-xs ${p1Wins ? "text-mc-grass" : "text-mc-stone-light"}`}
							>
								{r.p1Obtained.toLocaleString()}
							</span>
							<span
						...
						<div className="h-3 bg-black/40 border-2 border-mc-border flex">
							...
							/>
							<div
								className="h-full bg-mc-red/60"
								style={{ width: `${100 - p1Frac}%` }}
							/>
						</div>
					</div>
				);
			...
			{ROWS.map((r) => {
				...
				return (
					...
					>
						...
						<span
							className={`${PF} text-sm text-mc-gold uppercase flex-1 truncate`}
						>
							{formatGameLabel(r.game)}
						</span>
						<span
							className={`${PF} text-xs ${diff > 0 ? "text-mc-grass" : diff < 0 ? "text-mc-red" : "text-mc-stone-light"}`}
						>
							{winner === "tie"
								? "TIE"
								: `${winner} +${Math.abs(diff).toLocaleString()}`}
						</span>
					</div>
				);
			})}
			...
			{ROWS.map((r) => {
				...
				const Side = ({
					...
					return (
						...
						>
							<div
								className="rounded-sm shrink-0 border-2"
								style={{
									width: 40,
									height: 40,
									background: c,
							...
							>
								<p
									className={`${PF} text-xs uppercase tracking-wider truncate ${wins ? "text-mc-grass" : "text-mc-stone-light"}`}
								>
									{name}
								</p>
								<p
									className={`${PF} text-sm tabular-nums ${wins ? "text-mc-gold" : "text-mc-stone-light"}`}
								>
									{obt.toLocaleString()} AP
								</p>
								<div className="flex items-center gap-1.5 mt-0.5">
									<div
										className={`flex-1 h-1.5 bg-black/40 border border-mc-border/60 overflow-hidden ${right ? "flex flex-row-reverse" : ""}`}
									>
										<div
											className="h-full bg-mc-grass"
											style={{
									...
									<span className="text-[0.6rem] text-mc-stone-light tabular-nums shrink-0">
										{pct.toFixed(0)}%
									</span>
								</div>
							</div>
					...
					<div key={r.game} className={`${PANEL} p-3`}>
						<div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
							...
							<div className="flex flex-col items-center gap-1 px-2 border-x-2 border-mc-border/50">
								...
								<span
									className={`${PF} text-[0.65rem] text-mc-gold uppercase tracking-[0.03em] text-center leading-tight`}
								>
									{formatGameLabel(r.game)}
								</span>
							</div>
...
export default async function CompareShowcasePage() {
	...
	return (
		<main className="mx-auto max-w-3xl px-4 py-6 space-y-6">
			<h1
				className={`${PF} text-mc-gold text-2xl uppercase tracking-wide`}
			>
				Compare — Design Showcase
			</h1>
			<p className="text-sm text-mc-stone-light">
				All variants below use mock data. Tell me which letter you want
				for summary and breakdown.
			</p>
