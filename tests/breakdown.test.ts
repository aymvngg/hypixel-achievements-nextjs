...
function view(
	overrides: Partial<AchievementView> & Pick<AchievementView, "game">,
): AchievementView {
	const totalPoints = overrides.totalPoints ?? overrides.points ?? 10;
	const obtainedPoints =
		overrides.obtainedPoints ?? (overrides.completed ? totalPoints : 0);
...
describe("sortGameBreakdown", () => {
	...
	it("sorts by obtained points descending", () => {
		...
		expect(sorted.map((row) => row.game)).toEqual([
			"bedwars",
			"skyblock",
			"duels",
		]);
